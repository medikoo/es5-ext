...
memoize = module.exports = function () {
	var fn, mfn, options, length, resolver, method, cache, find, save, clear
	  , value, primitive, getPrimitiveId, lid;

	fn = callable(this);
	if (fn.memoized) {
		// Do not allow memoization of already memoized function
		return fn;
	}

	options = Object(arguments[0]);

	lid = ++id + '.' + this.toString().slice(0, 100);
	...
	mfn.clearCache = function () {
		...
		if (primitive) {
			delete cache[getPrimitiveId(alength, args)];
		} else {
			clear(alength, args);
		}
	};
