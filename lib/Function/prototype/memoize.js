...
memoize = module.exports = function () {
	var fn, mfn, options, length, resolver, method, cache, find, save, clear
	  , value, primitive, lid = ++id + '.' + this.toString().slice(0, 100);

	stats[lid] = { initial: 0, cached: 0 };
	...
	mfn = function () {
		var args, alength, argsLength, argsValue;
		...
		alength = (length === false) ? argsLength : length;

		if (primitive) {
			if (alength) {
				if (alength < argsLength) {
					argsValue = slice.call(args, 0, alength);
				} else if (alength > argsLength) {
					argsValue = toArray(args);
					push.apply(argsValue, Array(alength - argsLength));
				} else {
					argsValue = args;
				}
				argsValue = (alength > 1) ? join.call(argsValue, '\u0001') :
					(argsValue[0] + '\u0002');
			} else {
				argsValue = '';
			}
			if (cache.hasOwnProperty(argsValue)) {
				++stats[lid].cached;
				return cache[argsValue];
			} else {
				++stats[lid].initial;
				mfn.args = arguments;
